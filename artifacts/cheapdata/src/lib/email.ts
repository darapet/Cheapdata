...
async function getBrevoKey(): Promise<{ apiKey: string | null; senderEmail: string; senderName: string }> {
  const { data } = await supabase
    .from('system_settings')
    .select('brevo_api_key, brevo_sender_email, brevo_sender_name')
    .maybeSingle();
  return {
    apiKey: data?.brevo_api_key || null,
    senderEmail: data?.brevo_sender_email || 'noreply@cheapdatahub.com',
    senderName: data?.brevo_sender_name || 'CheapDataHub',
  };
}
...
export async function sendTransactionEmail(params: TransactionEmailParams): Promise<void> {
  const { apiKey, senderEmail, senderName } = await getBrevoKey();
  if (!apiKey) return;
          ...
          <td style="padding:32px;">
            ...
            <table width="100%" cellpadding="12" cellspacing="0" style="background:#f9fafb;border-radius:12px;margin-bottom:24px;">
              ...
              <tr>
                ...
                <td align="right"><span style="background:${params.status === 'successful' ? '#dcfce7' : '#fee2e2'};color:${statusColor};padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;">${statusLabel}</span></td>
              </tr>
            </table>
