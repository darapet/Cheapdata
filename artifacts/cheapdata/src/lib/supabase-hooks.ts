  ...
  return useMutation({
    mutationFn: async ({ data: { amount } }: { data: { amount: number } }) => {
      ...
      const { data: settings } = await supabase
        ...
        .select('cheapdatahub_funding_account')
        .maybeSingle();
      ...
      return {
        reference,
        total_amount,
        funding_account: settings?.cheapdatahub_funding_account ?? '0123456789',
      };
    },
  });
}
...
async function deductAndRecord(
  userId: string,
  balance: number,
  amount: number,
  type: string,
...
export function useBuyData() {
  return useMutation({
    mutationFn: async ({
      ...
      data: { phone: string; plan_id: string; network: string; pin: string };
    }) => {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) throw new Error('Not authenticated');
      ...
      if (balance < plan.retail_price) {
        return { success: false, message: 'Insufficient wallet balance. Please fund your wallet.' };
      }

      await deductAndRecord(
        user.id,
        balance,
        plan.retail_price,
        'data',
        `${data.network} ${plan.plan_name} → ${data.phone}`,
        makeRef('DATA'),
      );
...
export function useBuyAirtime() {
  return useMutation({
    mutationFn: async ({
      ...
      data: { phone: string; network: string; amount: number; pin: string };
    }) => {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) throw new Error('Not authenticated');
      ...
      await deductAndRecord(
        ...
        data.amount,
        'airtime',
        `${data.network} Airtime ₦${data.amount.toLocaleString()} → ${data.phone}`,
        makeRef('AIRTIME'),
      );

      return { success: true, message: `₦${data.amount.toLocaleString()} airtime sent to ${data.phone} successfully!` };
    },
  });
}
...
export function useBuyCable() {
  return useMutation({
    mutationFn: async ({
      ...
      data: { smart_card_number: string; cable_provider: string; plan_id: string; amount: number; pin: string };
    }) => {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) throw new Error('Not authenticated');
      ...
      await deductAndRecord(
        ...
        data.amount,
        'cable',
        `${data.cable_provider} Subscription → IUC: ${data.smart_card_number}`,
        makeRef('CABLE'),
      );
...
export function useBuyElectricity() {
  return useMutation({
    mutationFn: async ({
      ...
      data: { meter_number: string; disco: string; amount: number; meter_type: string; pin: string };
    }) => {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) throw new Error('Not authenticated');
      ...
      await deductAndRecord(
        ...
        data.amount,
        'electricity',
        `${data.disco} ${data.meter_type} Token → Meter: ${data.meter_number}`,
        makeRef('ELEC'),
      );
