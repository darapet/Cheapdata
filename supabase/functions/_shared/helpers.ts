...
export async function getUserProfile(supabaseAdmin: ReturnType<typeof getSupabaseAdmin>, userId: string) {
  const { data } = await supabaseAdmin.from('profiles').select('email, full_name, phone').eq('id', userId).single();
  return data;
}
...
/**
 * Call the CheapDataHub Reseller API.
 * NOTE: CheapDataHub returns { "status": "true" } — a STRING, not boolean true.
...
export async function cheapdatahubCall(
  ...
  // CheapDataHub returns status as the string "true" (not boolean true)
  const ok = res.ok && (
