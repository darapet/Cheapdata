import { corsHeaders, getSupabaseAdmin, verifyAuthToken, getSettings, jsonResponse } from '../_shared/helpers.ts';
  ...
  Deno.serve(async (req) => {
    ...
    try {
      ...
      const key = (settings?.paystack_secret_key as string | null)?.trim();
      if (!key) return jsonResponse({ error: 'No Paystack secret key saved. Save your key first.' }, 400);
      ...
      if (body.status) {
        return jsonResponse({ ok: true, message: 'Paystack key is valid ✓' });
      }
      return jsonResponse({ error: `Paystack rejected the key: ${body.message}` }, 400);
    } catch (err) {
