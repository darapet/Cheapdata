...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { smart_card_number, cable_provider, plan_id, pin } = await req.json() as {
      smart_card_number: string; cable_provider: string; plan_id: string; pin: string;
    };
    ...
    const wholesalePrice = Number(plan.wholesale_price ?? (retailPrice - 100));
    ...
    if (settings?.cheapdatahub_api_key) {
      try {
        const cdhPayload: Record<string, unknown> = {
          provider: String(cable_provider).toLowerCase(),
          smart_card_number,
