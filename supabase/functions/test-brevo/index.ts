import { corsHeaders, getSupabaseAdmin, verifyAuthToken, getSettings, jsonResponse } from '../_shared/helpers.ts';
  ...
  Deno.serve(async (req) => {
    ...
    try {
      ...
      if (!senderEmail) return jsonResponse({ error: 'Sender email not configured. Fill in the Sender Email field and save.' }, 400);
      ...
      return jsonResponse({ error: `Brevo error: ${err.message}` }, 400);
    } catch (err) {
